...
const formData = ref<FormData>({
  ...
  ttsModel: props.agent?.conversation_config?.tts?.model_id || 'eleven_multilingual_v2',
  ...
  maxDuration: (props.agent?.conversation_config?.conversation?.max_duration_seconds || 3600) / 60,
...
const handleSubmit = () => {
  const apiData = {
    ...
    conversation_config: {
      ...
      conversation: {
        max_duration_seconds: formData.value.maxDuration * 60
      }
    },
    platform_settings: {
      workspace_overrides: {
